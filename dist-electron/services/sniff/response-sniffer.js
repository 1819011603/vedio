...
exports.isHlsByContentType = isHlsByContentType;
exports.isVideoByContentType = isVideoByContentType;
exports.isHlsByBinaryHeader = isHlsByBinaryHeader;
exports.matchFilterRules = matchFilterRules;
exports.computeConfidence = computeConfidence;
exports.isValidVideoStream = isValidVideoStream;
 ...
 */
function isHlsByContentType(headers) {
    if (!headers)
        return false;
 ...
 */
function isVideoByContentType(headers) {
    if (!headers)
        return false;
    const ct = (headers['content-type'] || headers['Content-Type'] || '').toLowerCase();
 ...
 */
function isHlsByBinaryHeader(bodyPreview) {
    if (!bodyPreview || bodyPreview.length < 7)
        return false;
 ...
 */
function matchFilterRule(url, rule) {
 ...
 */
function matchFilterRules(url, rules) {
    if (!rules?.length)
        return { matched: true };
    for (const r of rules) {
        if (r.enabled === false)
            continue;
        if (matchFilterRule(url, r))
            return { matched: true, matchedRule: r.pattern };
    }
 ...
 */
function computeConfidence(meta, filterRules) {
    let score = 0;
    const reasons = [];
 ...
 * 判断是否为有效视频流响应
 */
function isValidVideoStream(meta, filterRules, options) {
 ...
 * 3. 取时间窗口内首个
 */
function selectBestCandidate(candidates, clickTime, timeWindow = [500, 3000]) {
