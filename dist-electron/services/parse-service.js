...
function isBilibiliPlaylist(url) {
...
function getDefaultHeadersForUrl(url) {
    const args = [];
    if (isBilibili(url)) {
...
function getBilibiliDefaultCookie(siteConfig) {
...
function buildExtraArgs(siteConfig, url) {
    const args = [];
    if (siteConfig) {
        if (siteConfig.cookie?.type === 'file' && siteConfig.cookie.filePath) {
...
class ParseService {
    async parseUrl(url, siteConfig, onOutput) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const proc = (0, child_process_1.spawn)(ytDlpPath, args, { stdio: ['pipe', 'pipe', 'pipe'] });
            let stdout = '';
            let stderr = '';
            const emit = (d, stream) => {
                const s = d.toString();
    ...
    parseYtDlpJson(obj, url) {
        ...
        const formats = (obj.formats || [])
            ...
            .filter((f) => f.resolution || f.note)
            .slice(0, 30);
        const videoUrl = String(obj.url || obj.webpage_url || url);
        const m3u8Format = (obj.formats || []).find((f) => /\.m3u8|m3u8|hls/i.test(f.url || ''));
        const m3u8Url = m3u8Format?.url || (/\.m3u8|m3u8/i.test(videoUrl) ? videoUrl : undefined);
        const formatsClean = formats.map((f) => ({
            id: String(f.id),
            ext: String(f.ext),
    ...
    async parsePlaylist(url, siteConfig, onOutput) {
        const ytDlpPath = await (0, path_utils_1.getYtDlpPath)();
        const extra = buildExtraArgs(siteConfig, url);
        return new Promise((resolve, reject) => {
    ...
    async parseMultipleUrls(urls, siteConfigs, onOutput) {
        const results = [];
