...
function getResourcesPath() {
    return electron_2.app.isPackaged ? (0, path_1.join)(process.resourcesPath) : (0, path_1.join)(process.cwd(), 'resources');
}
function sendOutput(webContents, type, line, stream) {
    if (webContents && !webContents.isDestroyed()) {
        webContents.send('parse-download-output', { type, line, stream });
    }
}
function registerIpcHandlers() {
    electron_1.ipcMain.handle('set-debug-output-enabled', (_, enabled) => {
        debugOutputEnabled = enabled;
    });
    electron_1.ipcMain.handle('parse-url', async (event, url, options) => {
        try {
            ...
            const onOutput = (options?.showDebugOutput && debugOutputEnabled)
                ? (line, stream) => sendOutput(event.sender, 'parse', line, stream)
                : undefined;
    ...
    electron_1.ipcMain.handle('parse-playlist', async (event, url, options) => {
        try {
            const siteConfig = options?.siteConfig && typeof options.siteConfig === 'object'
                ? JSON.parse(JSON.stringify(options.siteConfig))
                : null;
    ...
    electron_1.ipcMain.handle('parse-multiple-urls', async (event, urls, options) => {
        try {
    ...
    electron_1.ipcMain.handle('check-files-exist', (_, files) => {
    ...
    electron_1.ipcMain.handle('show-file-exists-dialog', async (_, existingPaths) => {
        ...
        return ['cancel', 'overwrite', 'rename'][result.response];
    });
    electron_1.ipcMain.handle('set-download-settings', (_, opts) => {
        if (typeof opts?.maxConcurrent === 'number' && opts.maxConcurrent >= 1) {
            downloadService.setMaxConcurrent(opts.maxConcurrent);
        }
    });
    electron_1.ipcMain.handle('download', async (_, task) => {
        return downloadService.addTask(task);
    });
    electron_1.ipcMain.handle('pause-download', async (_, id) => {
        return downloadService.pauseTask(id);
    });
    electron_1.ipcMain.handle('resume-download', async (_, id) => {
        return downloadService.resumeTask(id);
    });
    electron_1.ipcMain.handle('cancel-download', async (_, id) => {
        return downloadService.cancelTask(id);
    });
    electron_1.ipcMain.handle('get-download-progress', async () => {
        return downloadService.getProgress();
    });
    electron_1.ipcMain.handle('get-download-command', async (_, id) => {
    ...
    downloadService.on('output', (data) => {
    ...
    electron_1.ipcMain.handle('select-script-file', async () => {
        const result = await electron_1.dialog.showOpenDialog({
            properties: ['openFile'],
    ...
    electron_1.ipcMain.handle('get-app-path', () => electron_2.app.getPath('downloads'));
    electron_1.ipcMain.handle('fetch-image-with-headers', async (_, urlStr, referer, origin) => {
        try {
            ...
            await new Promise((resolve, reject) => {
                protocol.get(urlStr, { headers }, (res) => {
    ...
    electron_1.ipcMain.handle('rename-file', async (_, dirPath, oldBaseName, newBaseName) => {
        try {
            const fs = require('fs');
            const path = require('path');
    ...
    function getMainWindow() {
        return electron_1.BrowserWindow.getAllWindows().find((w) => !w.isDestroyed() && w.webContents && !w.webContents.isDestroyed() && !w.getTitle().includes('视频嗅探')) ?? null;
    }
    electron_1.ipcMain.handle('sniff-start', async (event, pageUrl, siteConfig) => {
        const raw = String(pageUrl || '').trim();
        if (!raw)
            throw new Error('请先输入要嗅探的链接');
        ...
        sniffSession?.stop();
        sniffSession = new sniff_1.SniffSession();
        const cfg = {
            domain: siteConfig?.domain || '',
            jsSelector: siteConfig?.sniff?.jsSelector || 'a[href*=".m3u8"], video source',
            actionScript: siteConfig?.sniff?.actionScript,
        ...
        await sniffSession.start(url, cfg, {
            ...
            onWindowClosed: () => {
                ...
                if (target && !target.isDestroyed()) {
                    try {
                        target.send('sniff-window-closed');
                    }
                    catch (_) { }
                }
            },
    ...
    electron_1.ipcMain.handle('sniff-click', async (event, selector, episodeLabel) => {
        if (!sniffSession)
            return { ok: false, error: '未启动嗅探' };
        const r = await sniffSession.clickAndSniff(selector, episodeLabel);
        const target = event.sender;
        if (target && !target.isDestroyed()) {
            try {
                target.send('sniff-result', r);
            }
            catch (_) { }
        }
        return { ok: true, result: r };
    });
    electron_1.ipcMain.handle('sniff-traverse', async (event, selector, options) => {
        if (!sniffSession)
            return { ok: false, error: '未启动嗅探' };
        const target = event.sender;
        const results = await sniffSession.traverseAndSniff(selector, (r) => {
            if (target && !target.isDestroyed()) {
                try {
                    target.send('sniff-result', r);
                }
    ...
    electron_1.ipcMain.handle('sniff-get-text-from-selector', async (_, selector) => {
        if (!sniffSession)
            return '';
        return sniffSession.getTextFromSelector(selector);
    });
    electron_1.ipcMain.handle('sniff-stop', () => {
        try {
            sniffSession?.stop();
        }
        finally {
    ...
    electron_1.ipcMain.handle('run-extract-script', async (event, url, scriptPath) => {
        ...
        const sendProgress = (msg) => {
            if (target && !target.isDestroyed()) {
                try {
                    target.send('parse-download-output', { type: 'extract', line: msg });
                }
                catch (_) { }
            }
        };
        ...
        try {
            const appPath = electron_2.app.isPackaged ? process.resourcesPath : process.cwd();
