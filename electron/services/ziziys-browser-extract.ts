...
function parseVideoUrl(url: string): { id: string; sid: string } | null {
  const m = url.match(/\/video\/(\d+)-(\d+)-(\d+)\.html/)
  if (!m) return null
  return { id: m[1], sid: m[2] }
}

const isValidVideoSource = (u: string) =>
  u && u.includes('play.kvmplay.org') && (u.includes('/ws/') || u.includes('/m3/') || u.endsWith('.m3u8') || u.endsWith('.svg'))

/** 从 player_aaaa 解析 url 及 POST 所需参数 expires/client/nonce/token */
function parsePlayerPage(data: string): { sourceUrl: string | null; body: Record<string, string | number> | null } {
  let sourceUrl: string | null = null
  let body: Record<string, string | number> | null = null

  const pick = (u: string | undefined) => {
    if (!u) return
    const s = (u || '').replace(/\\\//g, '/').trim()
    if (isValidVideoSource(s)) sourceUrl = s
  }
  ...
  if (!sourceUrl) {
    try {
      ...
      const u = json.url || json.source || (json as any).video_url
      if (u) {
        pick(u)
        if (sourceUrl) {
          body = {
  ...
  if (!sourceUrl && data.includes('play.kvmplay.org')) {
    ...
    if (m1) pick(m1[1])
    if (!sourceUrl && m2) pick('https://play.kvmplay.org/ws/' + m2[1] + '/' + m2[2])
  }
  if (!sourceUrl) {
    ...
    if (bodyMatch) {
      ...
      const re = /(\w+):\s*'([^']*)'/g
      let m
      while ((m = re.exec(bodyMatch[1])) !== null) (body as any)[m[1]] = m[2]
    }
    if (!sourceUrl && body?.source && isValidVideoSource(String(body.source))) sourceUrl = String(body.source)
  }

  return { sourceUrl, body }
}

function postToKvmplay(sourceUrl: string, body: Record<string, string | number> | null): Promise<{ url?: string; type?: string } | null> {
...
export async function runZiziysBrowserExtract(
  ...
  const sess = session.fromPartition(`ziziys-extract-${Date.now()}`)
  ...
  const logClose = (tag: string, extra?: string) => {
  ...
  try {
    ...
    const isWafPage = (h: string) =>
      h.includes('safeline') || h.includes('slg-') || h.includes('雷池') || /安全检测.*WAF|WAF.*驱动/i.test(h)
    ...
    // 解析标题和集数
    const pageInfo = await safeExec<{ title: string; episodeCount: number }>(`
      (function() {
        let title = '';
    ...
    for (let n = startN; n <= endN; n++) {
      ...
      try {
        const playerData = await safeExec<string>(`
